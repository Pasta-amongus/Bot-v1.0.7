...
print(f'  ')
print(color.CYAN + f' Simon_leaks ')
print(f'  ')
        ...
        if message.author.display_name in data['FullAccess']:
            ...
            if len(args) == 2:
                if args[1].lower() == 'add' or args[1].lower() == 'remove':
                    ...
                    print(f' [Simon_leaks] [{getTime()}] Please specify the name of the user you want to add/remove from the admin list')
                else:
                    await message.reply('Invalid usage, try !admin add <username> or !admin remove <username>')
                    print(f' [Simon_leaks] [{getTime()}] Invalid usage, try ' + color.GREEN + '!admin add <username> ' + color.END + 'or ' + color.GREEN + '!admin remove <username>' + color.END)
            if len(args) >= 3:
